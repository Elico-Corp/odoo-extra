...
def local_pgadmin_cursor():
    cnx = None
    try:
        # Add db_host, db_port, db_user and db_password since the ones passed
        # as ENV variables are not used by psycopg2
        cnx = psycopg2.connect(database="postgres", host=config['db_host'],
...
class runbot_build(osv.osv):
    ...
    def cmd(self, cr, uid, ids, context=None):
        ...
        for build in self.browse(cr, uid, ids, context=context):
            ...
            # Add db_host, db_port, db_user and db_password since the ones
            # passed as ENV variables are not used by psycopg2
            cmd += ["--db_host=%s" % config['db_host']]
            cmd += ["--db_port=%s" % config['db_port']]
            cmd += ["--db_user=%s" % config['db_user']]
            cmd += ["--db_password=%s" % config['db_password']]

            # Add db_maxconn in order to limit the connections of each instance
            cmd += ["--db_maxconn=16"]
